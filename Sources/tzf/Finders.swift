/// tzf for swift.
///
/// NOTE: This package use a simplified polygon data and not so accurate around
/// borders.
///
...
/// A protocol for timezone finders.
public protocol F {
  /// Returns the data version.
  func dataVersion() -> String
  /// Returns the timezone for a given longitude and latitude.
  func getTimezone(lng: Double, lat: Double) throws -> String
  /// Returns the timezones for a given longitude and latitude.
  func getTimezones(lng: Double, lat: Double) throws -> [String]
}

/// PreindexFinder will use map tiles(eg X/Y/Z) to find the timezone. The
/// underlying data is generated by a Go tool name [preindextzpb](https://github.com/ringsaturn/tzf/blob/main/cmd/preindextzpb/main.go).
/// And the data is stored in the bundle as `combined-with-oceans.reduce.preindex.bin`.
